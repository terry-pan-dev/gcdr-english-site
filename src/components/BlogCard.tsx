...
export function BlogCard({ post, index }: Props) {
  ...
  return (
    <motion.a
      ...
      } gap-3 sm:gap-6 p-3 sm:p-6 shadow-lg transition-all duration-300 hover:shadow-xl border-2 sm:border-[3px] cursor-pointer block`}
      style={{ backgroundColor: 'white', borderColor: '#c9a050', borderRadius: '0.75rem', textDecoration: 'none' }}
    >
      ...
      >
        ...
        <h2
          className={`font-serif mb-2 sm:mb-3 ${hasImage ? 'text-xl sm:text-2xl' : 'text-2xl sm:text-3xl'}`}
          style={{ color: '#1c1917' }}
        >
          {post.data.title}
        </h2>
        ...
        <p
          className={`text-stone-600 mb-3 sm:mb-4 text-sm sm:text-base ${hasImage ? 'line-clamp-2 sm:line-clamp-3' : 'line-clamp-2 sm:line-clamp-4'}`}
        >
          {post.data.excerpt}
        </p>
